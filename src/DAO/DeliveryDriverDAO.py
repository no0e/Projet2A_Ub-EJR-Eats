...
class DeliveryDriverDAO(UserDAO):
    ...
    def create(self, driver: DeliveryDriver) -> bool:
        try:
            self.db.sql_query(
                """
                INSERT INTO project_database.delivery_driver (username_delivery_driver, vehicle, is_available)
                VALUES (%s, %s, %s)
                """,
    ...
    def find_by_username(self, username: str):
        query = """
            ...
            JOIN project_database.delivery_driver d ON u.username = d.username_delivery_driver
            WHERE u.username = %s
        """
    ...
    def update(self, driver: DeliveryDriver) -> bool:
        query = """
            UPDATE project_database.delivery_driver
            SET vehicle = %s, is_available = %s
            WHERE username_delivery_driver = %s
        """
        self.db.sql_query(query, [driver.vehicle, driver.is_available, driver.username])
        return True

    def delete(self, driver: DeliveryDriver) -> bool:
        self.db.sql_query(
            "DELETE FROM project_database.delivery_driver WHERE username_delivery_driver = %s",
            [driver.username],
        )
        return True
